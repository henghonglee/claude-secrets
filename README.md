...
- **Output Redaction** — Automatically redacts known secrets plus built-in patterns (AWS keys, GitHub/Slack tokens, generic API keys, bearer tokens)
- **Secret Capture** — Extract secrets from command output (e.g., AWS session tokens) and store them for future use
- **LLM-Friendly Metadata** — Descriptions help future LLMs discover and use the right secrets
- **macOS Menu Bar App** — Native notifications and a secret-request prompt for missing secrets
- **Local Web UI** — Browser-based UI (bundled with the menu bar app) to add, view, and delete secrets
- **Encrypted Vault** — Secrets stored with Fernet (AES-128-CBC + HMAC-SHA256)

## Installation

Requires Python 3.11 or newer. The menu bar app, auto-start, and native dialogs are macOS-only; the MCP server and CLI also run on Linux.
...
A good description helps future LLMs understand what each secret is for, and tags let you group and filter related secrets.
...
| `ccs serve [--session-timeout 1h] [--port N]` | Start the MCP server over stdio (see note below) |
...
- Pages:
  ...
  - `/list` — table of all secrets with a status badge (**Valid** / **Expiring** within 24h / **Expired**) and a per-row **Delete** action
